...
void plot_histogram_hq(rplidar_response_measurement_node_hq_t * nodes, size_t count)
{
	const int BARCOUNT = 75;
	const int MAXBARHEIGHT = 20;
	const float ANGLESCALE = 360.0f / BARCOUNT;
	...
	float max_val = 0;
	for (int pos = 0; pos < (int)count; ++pos) {
		int int_deg = (int)((nodes[pos].angle_z_q14 >> RPLIDAR_RESP_MEASUREMENT_ANGLE_SHIFT) / 64.0f / ANGLESCALE);
		if (int_deg >= BARCOUNT) int_deg = 0;
		float cachedd = histogram[int_deg];
		if (cachedd == 0.0f) {
...
u_result capture_and_display_continuously(RPlidarDriver * drv, int times = 100) {
	u_result ans;

	rplidar_response_measurement_node_hq_t nodes[8192];
	size_t   count = _countof(nodes);

	printf("Plotting histogram... Press any key to continue.\n");
	getchar();
...
int main(int argc, const char * argv[]) {
	...
	do {
		printf("Capturing data x times? ");
		scanf("%d", &times);

		if (times <= 0) break;

		if (IS_FAIL(capture_and_display_continuously(drv, times))) {
			fprintf(stderr, "Error, cannot grab scan data.\n");
			break;
		}
	} while (times > 0);
